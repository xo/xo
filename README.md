...
`xo` provides a set of generic "base" templates for each of the supported
databases, but these templates are not suitable for every organization or every
schema out there. As such, you can author your own custom templates, or modify
the base templates available in the `xo` source tree, and use those with `xo`
by a passing a directory path via the `--template-path` flag.
...
Please see the [booktest example](examples/booktest) directory for a full
end-to-end example for each supported database, showcasing how to use a
database schema with `xo`, and the resulting code generated by `xo`.

Additionally, please see the [pokedex example](examples/pokedex) directory for
a demonstration of running `xo` against a large schema. Please note that this
...
need `xo` to ignore fields for a database schema, you can use the
`--ignore-fields` flag.

For example, a common use case is to define a table with `created_at` and/or
`modified_at` timestamps, where the database is responsible for setting column
values on `INSERT` and `UPDATE`, respectively. Consider the following
PostgreSQL schema where the `created_at` field has a default value of `now()`,
and where the the `users` table has a `UPDATE` trigger setting `modified_at`:
...
`created_at` and `modified_at` field (as in the example PostgreSQL schema in
the previous example). It is then a common use-case to have a `GetMostRecent`
...
First, we create a copy of the base `xo` templates, as such:
...
Now that we have a copy of the base `xo` templates, we can modify them to suit
our specific schema, adding lookups, helpers, or anything else necessary for
our schema.

In order to add a `GetMostRecent` lookup, we need to modify our database's
"type" template, which defines the Go type generated for each template. See the
above section on templates for more information about the available templates
and types. For our PostgreSQL database (see schema in previous example), that
is the `templates/postgres.type.go.tpl` file.

As such, we edit our copy of the `postgres.type.go.tpl` template:
...
We can then use our local templates in conjunction with `xo` to generate our
"model" code:
...
escaped using any of hte `--escape-*` options, you must pass the
`sql_mode=ansi` option to the MySQL driver:
...
#### Helper Scripts and Oracle Docker Image
