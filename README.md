...
If the database is named something other than 'public' it is necessary to provide the database name as the schema via the `-s name` argument when using `xo`:

```sh
$ xo cr://cr_user@localhost:26257/crdb?sslmode=disable -s crdb -o models
```
