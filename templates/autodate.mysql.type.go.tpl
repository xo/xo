...
func ({{ shortname .Name }} *{{ .Name }}) Insert(db XODB) error {
	...
	// run query
	XOLog(sqlstr, {{ fieldnameswithtime_insert .Fields (shortname .Name) .PrimaryKey.Name }})
	res, err := db.Exec(sqlstr, {{ fieldnameswithtime_insert .Fields (shortname .Name) .PrimaryKey.Name }})
	if err != nil {
		return err
	}
...
func ({{ shortname .Name }} *{{ .Name }}) Update(db XODB) error {
	...
	// run query
	XOLog(sqlstr, {{ fieldnameswithtime_update .Fields (shortname .Name) .PrimaryKey.Name }}, {{ shortname .Name }}.{{ .PrimaryKey.Name }})
	_, err = db.Exec(sqlstr, {{ fieldnameswithtime_update .Fields (shortname .Name) .PrimaryKey.Name }}, {{ shortname .Name }}.{{ .PrimaryKey.Name }})
	return err
}
