...
func (p *Product) Insert(ctx context.Context, db DB) error {
	...
	// run
	logf(sqlstr, p.ProductName, p.SupplierID, p.CategoryID, p.QuantityPerUnit, p.UnitPrice, p.UnitsInStock, p.UnitsOnOrder, p.ReorderLevel, p.Discontinued, nil)
	var id int64
	if _, err := db.ExecContext(ctx, sqlstr, p.ProductName, p.SupplierID, p.CategoryID, p.QuantityPerUnit, p.UnitPrice, p.UnitsInStock, p.UnitsOnOrder, p.ReorderLevel, p.Discontinued, sql.Named("pk", sql.Out{Dest: &id})); err != nil {
		return err
	} // set primary key
	p.ProductID = int(id)
	// set exists
...
func (p *Product) Delete(ctx context.Context, db DB) error {
	...
	const sqlstr = `DELETE FROM northwind.products WHERE product_id = :1`
	// run
