...
// BooktestAuthor represents a row from 'booktest_author'.
...
func (ba *BooktestAuthor) Insert(ctx context.Context, db DB) error {
	...
	const sqlstr = `INSERT INTO booktest_author (` +
		`author_id, name` +
		`) VALUES (` +
		`$1, $2` +
		`)`
	// run
	logf(sqlstr, ba.Name)
	res, err := db.ExecContext(ctx, sqlstr, ba.AuthorID, ba.Name)
	if err != nil {
		return err
	}
	// retrieve id
...
func (ba *BooktestAuthor) Update(ctx context.Context, db DB) error {
	...
	const sqlstr = `UPDATE booktest_author SET ` +
		`name = $1 ` +
		`WHERE author_id = $2`
	// run
...
func (ba *BooktestAuthor) Delete(ctx context.Context, db DB) error {
	...
	const sqlstr = `DELETE FROM booktest_author ` +
		`WHERE author_id = $1`
	// run
...
// BooktestAuthorByAuthorID retrieves a row from 'booktest_author' as a BooktestAuthor.
//
// Generated from index 'booktest_author_author_id_pkey'.
func BooktestAuthorByAuthorID(ctx context.Context, db DB, authorID int) (*BooktestAuthor, error) {
	// query
	const sqlstr = `SELECT ` +
		`author_id, name ` +
		`FROM booktest_author ` +
		`WHERE author_id = $1`
	// run
