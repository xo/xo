...
// BooktestBookTag represents a row from 'booktest_book_tags'.
...
func (bbt *BooktestBookTag) Insert(ctx context.Context, db DB) error {
	...
	const sqlstr = `INSERT INTO booktest_book_tags (` +
		`id, book_id, tag_id` +
		`) VALUES (` +
		`$1, $2, $3` +
		`)`
	...
	res, err := db.ExecContext(ctx, sqlstr, bbt.ID, bbt.BookID, bbt.TagID)
	if err != nil {
		return err
	}
	// retrieve id
...
func (bbt *BooktestBookTag) Update(ctx context.Context, db DB) error {
	...
	const sqlstr = `UPDATE booktest_book_tags SET ` +
		`book_id = $1, tag_id = $2 ` +
		`WHERE id = $3`
	// run
...
func (bbt *BooktestBookTag) Delete(ctx context.Context, db DB) error {
	...
	const sqlstr = `DELETE FROM booktest_book_tags ` +
		`WHERE id = $1`
	// run
	logf(sqlstr, bbt.ID)
...
// BooktestBookTagsByBookID retrieves a row from 'booktest_book_tags' as a BooktestBookTag.
//
// Generated from index 'booktest_book_tags_book_id_04f02754'.
func BooktestBookTagsByBookID(ctx context.Context, db DB, bookID int64) ([]*BooktestBookTag, error) {
	// query
	const sqlstr = `SELECT ` +
		`id, book_id, tag_id ` +
		`FROM booktest_book_tags ` +
		`WHERE book_id = $1`
	// run
	logf(sqlstr, bookID)
...
// BooktestBookTagByBookIDTagID retrieves a row from 'booktest_book_tags' as a BooktestBookTag.
//
...
// BooktestBookTagByID retrieves a row from 'booktest_book_tags' as a BooktestBookTag.
//
...
// BooktestBookTagsByTagID retrieves a row from 'booktest_book_tags' as a BooktestBookTag.
//
// Generated from index 'booktest_book_tags_tag_id_858da6e5'.
