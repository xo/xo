...
// BooktestTag represents a row from 'booktest_tag'.
...
func (bt *BooktestTag) Insert(ctx context.Context, db DB) error {
	...
	const sqlstr = `INSERT INTO booktest_tag (` +
		`tag_id, tag` +
		`) VALUES (` +
		`$1, $2` +
		`)`
	// run
	logf(sqlstr, bt.Tag)
	res, err := db.ExecContext(ctx, sqlstr, bt.TagID, bt.Tag)
	if err != nil {
		return err
	}
	// retrieve id
...
func (bt *BooktestTag) Update(ctx context.Context, db DB) error {
	...
	const sqlstr = `UPDATE booktest_tag SET ` +
		`tag = $1 ` +
		`WHERE tag_id = $2`
	// run
...
func (bt *BooktestTag) Delete(ctx context.Context, db DB) error {
	...
	const sqlstr = `DELETE FROM booktest_tag ` +
		`WHERE tag_id = $1`
	// run
...
// BooktestTagByTagID retrieves a row from 'booktest_tag' as a BooktestTag.
//
// Generated from index 'booktest_tag_tag_id_pkey'.
func BooktestTagByTagID(ctx context.Context, db DB, tagID int) (*BooktestTag, error) {
	// query
	const sqlstr = `SELECT ` +
		`tag_id, tag ` +
		`FROM booktest_tag ` +
