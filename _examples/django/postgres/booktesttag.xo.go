...
// BooktestTag represents a row from 'public.booktest_tag'.
...
func (bt *BooktestTag) Insert(ctx context.Context, db DB) error {
	...
	const sqlstr = `INSERT INTO public.booktest_tag (` +
		`tag` +
		`) VALUES (` +
		`$1` +
		`) RETURNING tag_id`
	// run
	logf(sqlstr, bt.Tag)
	if err := db.QueryRowContext(ctx, sqlstr, bt.Tag).Scan(&bt.TagID); err != nil {
		return logerror(err)
	}
	// set exists
	bt._exists = true
...
func (bt *BooktestTag) Update(ctx context.Context, db DB) error {
	...
	const sqlstr = `UPDATE public.booktest_tag SET ` +
		`tag = $1 ` +
		`WHERE tag_id = $2`
	// run
...
func (bt *BooktestTag) Delete(ctx context.Context, db DB) error {
	...
	const sqlstr = `DELETE FROM public.booktest_tag ` +
		`WHERE tag_id = $1`
	// run
...
// BooktestTagByTagID retrieves a row from 'public.booktest_tag' as a BooktestTag.
//
// Generated from index 'booktest_tag_pkey'.
func BooktestTagByTagID(ctx context.Context, db DB, tagID int64) (*BooktestTag, error) {
	// query
	const sqlstr = `SELECT ` +
		`tag_id, tag ` +
		`FROM public.booktest_tag ` +
		`WHERE tag_id = $1`
	// run
	logf(sqlstr, tagID)
	bt := BooktestTag{
