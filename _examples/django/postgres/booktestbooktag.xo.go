...
// BooktestBookTag represents a row from 'public.booktest_book_tags'.
...
func (bbt *BooktestBookTag) Insert(ctx context.Context, db DB) error {
	...
	const sqlstr = `INSERT INTO public.booktest_book_tags (` +
		`book_id, tag_id` +
		`) VALUES (` +
		`$1, $2` +
		`) RETURNING id`
	...
	if err := db.QueryRowContext(ctx, sqlstr, bbt.BookID, bbt.TagID).Scan(&bbt.ID); err != nil {
		return logerror(err)
	}
	// set exists
	bbt._exists = true
...
func (bbt *BooktestBookTag) Update(ctx context.Context, db DB) error {
	...
	const sqlstr = `UPDATE public.booktest_book_tags SET ` +
		`book_id = $1, tag_id = $2 ` +
		`WHERE id = $3`
	// run
...
func (bbt *BooktestBookTag) Delete(ctx context.Context, db DB) error {
	...
	const sqlstr = `DELETE FROM public.booktest_book_tags ` +
		`WHERE id = $1`
	// run
	logf(sqlstr, bbt.ID)
...
// BooktestBookTagsByBookID retrieves a row from 'public.booktest_book_tags' as a BooktestBookTag.
//
...
func BooktestBookTagsByBookID(ctx context.Context, db DB, bookID int64) ([]*BooktestBookTag, error) {
	...
	const sqlstr = `SELECT ` +
		`id, book_id, tag_id ` +
		`FROM public.booktest_book_tags ` +
		`WHERE book_id = $1`
	// run
	logf(sqlstr, bookID)
...
// BooktestBookTagByBookIDTagID retrieves a row from 'public.booktest_book_tags' as a BooktestBookTag.
//
...
// BooktestBookTagByID retrieves a row from 'public.booktest_book_tags' as a BooktestBookTag.
//
// Generated from index 'booktest_book_tags_pkey'.
...
// BooktestBookTagsByTagID retrieves a row from 'public.booktest_book_tags' as a BooktestBookTag.
//
